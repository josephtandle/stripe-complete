...
// 1. Payment Intents
// ---------------------------------------------------------------------------
server.tool("create_payment_intent", "Create a new Stripe PaymentIntent to collect a payment. Example: create a $50 USD payment intent for customer cus_abc123 with card payment method.", {
    ...
    if (metadata)
        Object.entries(metadata).forEach(([k, v]) => { body[`metadata[${k}]`] = v; });
...
server.tool("confirm_payment_intent", "Confirm a PaymentIntent to capture funds. Example: confirm pi_abc123 with payment method pm_card_visa.", {
...
server.tool("cancel_payment_intent", "Cancel a PaymentIntent that has not yet been captured. Example: cancel pi_abc123 because customer changed their mind.", {
...
server.tool("list_payment_intents", "List recent PaymentIntents, optionally filtered by customer. Example: list the last 20 payment intents for customer cus_abc123.", {
...
// ---------------------------------------------------------------------------
server.tool("create_customer", "Create a new Stripe customer record. Example: create customer John Doe with email john@example.com and phone +1555123456.", {
...
server.tool("get_customer", "Retrieve a single Stripe customer by ID. Example: get details for customer cus_abc123 including their subscriptions.", {
...
server.tool("update_customer", "Update a Stripe customer's details. Example: update customer cus_abc123 to change their email or add metadata.", {
    customer_id: z.string().describe("Stripe customer ID. Example: 'cus_NffrFeUfNV2Hib'"),
...
server.tool("list_customers", "List Stripe customers, optionally filtered by email. Example: find all customers with email containing @acme.com.", {
...
server.tool("delete_customer", "Permanently delete a Stripe customer and cancel all their subscriptions. Example: delete test customer cus_test123.", {
...
// 3. Subscriptions
// ---------------------------------------------------------------------------
server.tool("create_subscription", "Create a new subscription for a customer. Example: subscribe customer cus_abc123 to price price_monthly_pro at $29/month.", {
...
server.tool("update_subscription", "Update a subscription — change plan, quantity, or apply discounts. Example: upgrade sub_abc123 to a higher-tier price.", {
    ...
    if (price_id || quantity !== undefined) {
        ...
        if (itemId) {
            if (price_id)
                body[`items[0][id]`] = itemId, body[`items[0][price]`] = price_id;
...
server.tool("cancel_subscription", "Cancel a subscription immediately or at the end of the billing period. Example: cancel sub_abc123 at period end.", {
...
server.tool("list_subscriptions", "List subscriptions, optionally filtered by customer or status. Example: list all active subscriptions for customer cus_abc123.", {
...
// 4. Invoices
// ---------------------------------------------------------------------------
server.tool("create_invoice", "Create a draft invoice for a customer. Example: create an invoice for customer cus_abc123 for consulting services.", {
    customer: z.string().describe("Stripe customer ID. Example: 'cus_NffrFeUfNV2Hib'"),
...
server.tool("send_invoice", "Send a finalized invoice to the customer via email. Example: send invoice inv_abc123 to the customer.", {
...
server.tool("list_invoices", "List invoices, optionally filtered by customer or status. Example: list all unpaid invoices for the past 30 days.", {
    customer: z.string().optional().describe("Filter by customer ID"),
...
server.tool("get_invoice", "Retrieve a single invoice by ID with all line items and payment details. Example: get full details for invoice in_abc123.", {
    invoice_id: z.string().describe("Invoice ID. Example: 'in_1OkL2M2eZvKYlo2C1234'"),
}, async ({ invoice_id }) => {
    const invoice = await stripeGet(`/invoices/${invoice_id}`);
    return { content: [{ type: "text", text: JSON.stringify(invoice, null, 2) }] };
});
server.tool("pay_invoice", "Attempt to pay an open invoice immediately. Example: manually trigger payment for overdue invoice in_abc123.", {
...
// 5. Coupons & Discounts
// ---------------------------------------------------------------------------
server.tool("create_coupon", "Create a discount coupon. Example: create SUMMER25 coupon for 25% off, valid forever, max 100 redemptions.", {
...
server.tool("list_coupons", "List all coupons in your Stripe account. Example: show me all active discount coupons.", {
    limit: z.number().int().min(1).max(100).default(20).describe("Number of results. Default: 20"),
    starting_after: z.string().optional().describe("Cursor for pagination"),
...
server.tool("delete_coupon", "Delete a coupon by ID. This does not affect existing subscriptions using the coupon. Example: delete expired coupon SUMMER25.", {
...
server.tool("apply_coupon", "Apply a coupon discount to an existing customer or subscription. Example: apply VIP20 coupon to customer cus_abc123.", {
...
server.tool("create_promotion_code", "Create a shareable promotion code linked to a coupon. Example: create promo code LAUNCH50 for 50% off coupon, limited to 200 uses.", {
...
// 6. Products & Prices
// ---------------------------------------------------------------------------
server.tool("create_product", "Create a new product in Stripe. Example: create a 'Pro Plan' product for SaaS monthly subscription.", {
...
server.tool("create_price", "Create a price for a product (one-time or recurring). Example: create $29/month recurring price for product prod_abc123.", {
...
server.tool("list_products", "List all products in your Stripe catalog. Example: show me all active products.", {
...
server.tool("list_prices", "List prices, optionally filtered by product. Example: show all prices for product prod_abc123.", {
...
// 7. Refunds
// ---------------------------------------------------------------------------
server.tool("create_refund", "Issue a refund for a charge or payment intent. Example: refund $25 of charge ch_abc123 due to customer complaint.", {
    ...
    if (!charge && !payment_intent)
        throw new Error("Either charge or payment_intent is required");
    const body = {};
    if (charge)
        body.charge = charge;
    if (payment_intent)
...
server.tool("list_refunds", "List refunds, optionally filtered by charge. Example: show all refunds issued in the past month.", {
...
// 8. Reports / Balance
// ---------------------------------------------------------------------------
server.tool("get_balance", "Get your current Stripe account balance, including available and pending funds. Example: what is my current Stripe balance?", {}, async () => {
    const balance = await stripeGet("/balance");
    return { content: [{ type: "text", text: JSON.stringify(balance, null, 2) }] };
});
server.tool("list_balance_transactions", "List balance transactions (charges, refunds, payouts, fees). Example: show me all balance transactions from the past 7 days.", {
...
server.tool("create_report_run", "Kick off a Stripe Sigma report run and get the download URL when complete. Example: run a monthly revenue report for March 2024.", {
